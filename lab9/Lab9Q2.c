...
void pop(struct stack *s)
{
    ...
    {
        temp = dequeue(&s->q1);
        printf("\nElement %d has been Popped from stack", temp);
    }
}
...
// Enqueues/Inserts an element to the End of desired Queue
void enqueue(struct node **front, int data)
{
...
// Dequeues/Deletes an element from the Front of desired Queue
int dequeue(struct node **front)
{
    int temp;
...
// Finds the top/front element of the desired Queue
int peek(struct stack *s, struct node **front)
{
    int temp;
    temp = (*front)->val;
